...
public:
    TraceLogger(const std::string& filename = "configured_pipeline_trace.json") 
...
// Simple YAML-like configuration parser
class ConfigParser {
public:
    struct StageConfig {
        std::string name;
    ...
    static PipelineConfig parseConfig(const std::string& configContent) {
        PipelineConfig config;
        
        // For this example, create a simple pipeline config programmatically
        // In a real implementation, you would parse actual YAML/JSON
        
        // Create a sample pipeline with generation, validation, transformation, and rendering stages
        StageConfig genStage;
        genStage.name = "Generation";
        genStage.workers = 2;
        
        std::map<std::string, std::string> genTask;
        genTask["id"] = "generate_circle_1";
        genTask["type"] = "GenerateShape";
        genTask["shapeType"] = "circle";
        genTask["param1"] = "2.0";
        genTask["posX"] = "0.0";
        genTask["posY"] = "0.0";
        genStage.tasks.push_back(genTask);
        
        std::map<std::string, std::string> genTask2;
        genTask2["id"] = "generate_rect_1";
        genTask2["type"] = "GenerateShape";
        genTask2["shapeType"] = "rectangle";
        genTask2["param1"] = "3.0";
        genTask2["param2"] = "4.0";
        genTask2["posX"] = "10.0";
        genTask2["posY"] = "10.0";
        genStage.tasks.push_back(genTask2);
        
        config.stages.push_back(genStage);
        
        StageConfig valStage;
        valStage.name = "Validation";
        valStage.workers = 2;
        
        std::map<std::string, std::string> valTask;
        valTask["id"] = "validate_circle_1";
        valTask["type"] = "ValidateShape";
        valStage.tasks.push_back(valTask);
        
        std::map<std::string, std::string> valTask2;
        valTask2["id"] = "validate_rect_1";
        valTask2["type"] = "ValidateShape";
        valStage.tasks.push_back(valTask2);
        
        config.stages.push_back(valStage);
        
        StageConfig transStage;
        transStage.name = "Transformation";
        transStage.workers = 1;
        
        std::map<std::string, std::string> transTask;
        transTask["id"] = "transform_circle_1";
        transTask["type"] = "TransformShape";
        transTask["scaleX"] = "2.0";
        transTask["scaleY"] = "2.0";
        transStage.tasks.push_back(transTask);
        
        config.stages.push_back(transStage);
        
        StageConfig renderStage;
        renderStage.name = "Rendering";
        renderStage.workers = 1;
        
        std::map<std::string, std::string> renderTask;
        renderTask["id"] = "render_circle_1";
        renderTask["type"] = "RenderShape";
        renderStage.tasks.push_back(renderTask);
        
        config.stages.push_back(renderStage);
        
        // Define dependencies: validate after generate, transform after validate, render after transform
        config.dependencies.push_back({"validate_circle_1", "generate_circle_1"});  // validate depends on generate
        config.dependencies.push_back({"validate_rect_1", "generate_rect_1"});    // validate depends on generate
        config.dependencies.push_back({"transform_circle_1", "validate_circle_1"}); // transform depends on validate
        config.dependencies.push_back({"render_circle_1", "transform_circle_1"});   // render depends on transform
        
        config.name = "Configured Shape Pipeline";
...
public:
    ...
    void onTaskCompleted(const std::string& completedTaskId) {
        std::lock_guard<std::mutex> lock(schedulerMutex);
    ...
    void waitForCompletion() {
        while (true) {
            {
                std::lock_guard<std::mutex> lock(schedulerMutex);
                if (getCompletedTaskCount() == taskMap.size()) {
                    break;
                }
            }
...
public:
    ...
    // Create pipeline from configuration content (instead of file for simplicity)
    static std::shared_ptr<ConfigurablePipeline> createFromConfigContent(const std::string& configContent) {
        auto config = ConfigParser::parseConfig(configContent);
    ...
    static std::shared_ptr<Task> createTaskFromConfig(const std::string& taskId, 
        ...
        if (taskType == "GenerateShape") {
            ...
            auto paramIt = config.find("param1");
            if (paramIt != config.end()) param1 = std::stod(paramIt->second);
            paramIt = config.find("param2");
            if (paramIt != config.end()) param2 = std::stod(paramIt->second);
            paramIt = config.find("posX");
            if (paramIt != config.end()) posX = std::stod(paramIt->second);
            paramIt = config.find("posY");
            if (paramIt != config.end()) posY = std::stod(paramIt->second);
            ...
            auto scaleXIt = config.find("scaleX");
            if (scaleXIt != config.end()) scaleX = std::stod(scaleXIt->second);
            auto scaleYIt = config.find("scaleY");
            if (scaleYIt != config.end()) scaleY = std::stod(scaleYIt->second);
...
int main() {
    ...
    // Example configuration content (in a real system this would be in a YAML/JSON file)
    std::string configContent = R"(
        This is a sample configuration that would normally be in a YAML or JSON file.
        The actual implementation would read from a file.
    )";
    
    try {
        // Create pipeline from configuration
        auto pipeline = ConfigurablePipeline::createFromConfigContent(configContent);

        std::cout << "Pipeline created from configuration: " << pipeline->getName() << std::endl;
