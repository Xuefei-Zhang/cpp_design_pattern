...
public:
    TraceLogger(const std::string& filename = "json_config_pipeline_trace.json") 
...
public:
    struct PipelineConfig {
        std::string name;
        std::vector<json> stages;  // Store raw JSON for stages
        std::vector<std::pair<std::string, std::string>> dependencies; // from -> to
    };
    
    static PipelineConfig parseJsonConfig(const std::string& configFile) {
        ...
        config.name = j.value("name", "Default Pipeline");
        
        // Store stages as JSON objects
        ...
        if (j.contains("dependencies") && j["dependencies"].is_array()) {
            for (const auto& dep : j["dependencies"]) {
                if (dep.is_array() && dep.size() == 2) {
                    config.dependencies.push_back({dep[0].get<std::string>(), dep[1].get<std::string>()});
                }
            }
        }
...
public:
    ...
    static std::shared_ptr<ConfigurablePipeline> createFromConfigFile(const std::string& configFile) {
        auto config = ConfigParser::parseJsonConfig(configFile);
        
        auto pipeline = std::make_shared<ConfigurablePipeline>(config.name, 4); // Default 4 executors
        
        // Create tasks based on configuration
        std::map<std::string, std::shared_ptr<Shape>> shapeRegistry; // To pass shapes between tasks
        
        for (const auto& stage : config.stages) {
            if (stage.contains("tasks") && stage["tasks"].is_array()) {
                for (const auto& taskJson : stage["tasks"]) {
                    std::string taskId = taskJson.value("id", "");
                    std::string taskType = taskJson.value("type", "");
                    
                    // Create appropriate task based on type
                    std::shared_ptr<Task> task = createTaskFromJson(taskId, taskType, taskJson);
                    if (task) {
    ...
    static std::shared_ptr<Task> createTaskFromJson(const std::string& taskId, 
                                                    const std::string& taskType,
                                                    const json& taskJson) {
        if (taskType == "GenerateShape") {
            ShapeFactory::ShapeType shapeType = ShapeFactory::ShapeType::CIRCLE;
            double param1 = taskJson.value("param1", 1.0);
            double param2 = taskJson.value("param2", 1.0);
            double posX = taskJson.value("posX", 0.0);
            double posY = taskJson.value("posY", 0.0);
            
            std::string shapeTypeStr = taskJson.value("shapeType", "circle");
            ...
            double scaleX = taskJson.value("scaleX", 1.0);
            double scaleY = taskJson.value("scaleY", 1.0);
...
// Example usage
int main() {
    std::cout << "JSON Configurable Pipeline System with Shapes Example\n" << std::endl;
    ...
    try {
        ...
        std::cout << "\nJSON Configurable Pipeline System demonstration completed!" << std::endl;
