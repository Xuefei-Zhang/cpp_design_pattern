...
public:
    TraceLogger(const std::string& filename = "yaml_config_pipeline_trace.json") 
...
// Simple configuration parser for JSON-style format
class ConfigParser {
public:
    struct PipelineConfig {
        std::string name;
        std::vector<std::map<std::string, std::string>> stages;  // Simplified stage info
        std::vector<std::map<std::string, std::string>> tasks;   // All tasks
        std::vector<std::pair<std::string, std::string>> dependencies; // from -> to
    };
    
    static PipelineConfig parseConfigFile(const std::string& configFile) {
        ...
        // This is a simplified parser - in a real implementation you'd use a proper JSON/YAML library
        // For this example, we'll just hardcode the expected structure matching our JSON file
        
        // Since this is a demo, we'll return a hardcoded config that matches our JSON file
        config.name = "Shape Processing Pipeline";
        
        // Add tasks based on the expected JSON structure
        std::map<std::string, std::string> genCircleTask;
        genCircleTask["id"] = "generate_circle_1";
        genCircleTask["type"] = "GenerateShape";
        genCircleTask["shapeType"] = "circle";
        genCircleTask["param1"] = "2.0";
        genCircleTask["posX"] = "0.0";
        genCircleTask["posY"] = "0.0";
        config.tasks.push_back(genCircleTask);
        
        std::map<std::string, std::string> genRectTask;
        genRectTask["id"] = "generate_rect_1";
        genRectTask["type"] = "GenerateShape";
        genRectTask["shapeType"] = "rectangle";
        genRectTask["param1"] = "3.0";
        genRectTask["param2"] = "4.0";
        genRectTask["posX"] = "10.0";
        genRectTask["posY"] = "10.0";
        config.tasks.push_back(genRectTask);
        
        std::map<std::string, std::string> valCircleTask;
        valCircleTask["id"] = "validate_circle_1";
        valCircleTask["type"] = "ValidateShape";
        config.tasks.push_back(valCircleTask);
        
        std::map<std::string, std::string> valRectTask;
        valRectTask["id"] = "validate_rect_1";
        valRectTask["type"] = "ValidateShape";
        config.tasks.push_back(valRectTask);
        
        std::map<std::string, std::string> transCircleTask;
        transCircleTask["id"] = "transform_circle_1";
        transCircleTask["type"] = "TransformShape";
        transCircleTask["scaleX"] = "2.0";
        transCircleTask["scaleY"] = "2.0";
        config.tasks.push_back(transCircleTask);
        
        std::map<std::string, std::string> renderCircleTask;
        renderCircleTask["id"] = "render_circle_1";
        renderCircleTask["type"] = "RenderShape";
        config.tasks.push_back(renderCircleTask);
...
public:
    ...
    static std::shared_ptr<ConfigurablePipeline> createFromConfigFile(const std::string& configFile) {
        auto config = ConfigParser::parseConfigFile(configFile);
        
        auto pipeline = std::make_shared<ConfigurablePipeline>(config.name, 4); // Default 4 executors
        
        // Create tasks based on configuration
        for (const auto& taskInfo : config.tasks) {
    ...
    static std::shared_ptr<Task> createTaskFromConfig(const std::string& taskId, 
        ...
        if (taskType == "GenerateShape") {
            ...
            double param1 = std::stod(config.at("param1"));
            double param2 = config.count("param2") ? std::stod(config.at("param2")) : 1.0;
            double posX = std::stod(config.at("posX"));
            double posY = std::stod(config.at("posY"));
            
            std::string shapeTypeStr = config.at("shapeType");
            if (shapeTypeStr == "rectangle") shapeType = ShapeFactory::ShapeType::RECTANGLE;
            else if (shapeTypeStr == "triangle") shapeType = ShapeFactory::ShapeType::TRIANGLE;
            ...
            double scaleX = std::stod(config.at("scaleX"));
            double scaleY = config.count("scaleY") ? std::stod(config.at("scaleY")) : 1.0;
...
// Example usage
int main() {
    std::cout << "YAML/JSON Configurable Pipeline System with Shapes Example\n" << std::endl;
    
    try {
        // Create pipeline from configuration file
        auto pipeline = ConfigurablePipeline::createFromConfigFile("examples/pipelines/shape_pipeline_config.json");
        ...
        std::cout << "\nYAML/JSON Configurable Pipeline System demonstration completed!" << std::endl;
