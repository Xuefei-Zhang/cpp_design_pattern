...
int main() {
    ...
    if (complexChain) {
        std::cout << "Complex chain created with multiple handlers\n";
    ...
    // Create a chain with only validation and logging handlers (no styling handler)
    ...
    for (size_t i = 0; i < requests.size(); ++i) {
        ...
        // Note: We can't reuse the same chain because it's moved during processing
        // In a real implementation, chains would be configured differently
    ...
    // Test 13: Performance Comparison - This would be done in a real scenario
    ...
    std::cout << "18. Chain Termination and Response Handling:\n";
    ...
    std::cout << "Processed 100 operations through chains in: " << duration.count() << " microseconds\n";
    std::cout << "Average time per operation: " << (duration.count() / 100.0) << " microseconds\n";
    std::cout << std::endl;
    ...
    // Test 22: Comparison with Direct Method Calls
    std::cout << "22. Comparison: Chain of Responsibility vs Direct Method Calls:\n";
    
    std::cout << "\nDirect Method Calls:\n";
    ShapeOperationRequest directRequest(ShapeOperationRequest::OperationType::DRAW, "DirectCircle", 5.0, 0.0, 0.0, "direct_color");
    std::cout << "Direct simulation: Drawing shape with basic parameters\n";
    std::cout << "Direct area calculation would be: 3.14159 * 5.0 * 5.0 = 78.54\n";

    std::cout << "\nChain of Responsibility:\n";
    auto chainRequest = ShapeOperationRequest(ShapeOperationRequest::OperationType::DRAW, "ChainCircle", 5.0, 0.0, 0.0, "chain_color");
    auto chain = ShapeHandlerChainBuilder()
                    .addValidationHandler()
                    .addLoggingHandler()
                    .addDrawHandler()
                    .buildChain();
    chain->handleRequest(chainRequest);
    std::cout << std::endl;
    
    std::cout << "Benefits of Chain of Responsibility:\n";
    std::cout << "- Handles complex processing workflows without client knowledge\n";
    std::cout << "- Allows for flexible configuration of processing steps\n";
    std::cout << "- Enables dynamic modification of processing behavior\n";
    std::cout << "- Decouples the request sender from the specific processor\n";
    std::cout << std::endl;
    
    std::cout << "Chain of Responsibility pattern demo completed successfully!\n";
    std::cout << "\nKey Benefits of Chain of Responsibility Pattern:\n";
    std::cout << "- Decouples the sender of a request from its receiver\n";
    std::cout << "- Gives more than one object a chance to handle the request\n";
    std::cout << "- Chains the receiving objects and passes the request along the chain until an object handles it\n";
    std::cout << "- Enables flexible request handling through dynamic chain configuration\n";
    std::cout << "- Promotes loose coupling in your system\n";
    std::cout << "- Makes it easier to introduce new request handlers\n";
