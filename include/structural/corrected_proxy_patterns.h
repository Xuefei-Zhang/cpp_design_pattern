#include <iostream>
#include <string>
#include <memory>
#include <vector>
#include <stack>
#include <functional>
#include <algorithm>
#include <map>
#include <chrono>
#include <thread>

// Subject interface - common interface for RealSubject and Proxy
class Shape {
public:
    virtual ~Shape() = default;
    virtual void draw() const = 0;
    virtual std::string getType() const = 0;
    virtual double getArea() const = 0;
    virtual void printInfo() const = 0;
};

// Real subjects - the actual business objects
class Circle : public Shape {
private:
    double radius_;
    std::string color_;
    std::string name_;

public:
    explicit Circle(double radius, const std::string& color = "red", const std::string& name = "Circle") 
        : radius_(radius), color_(color), name_(name) {
        std::cout << "[CIRCLE] Created: " << name_ << " (r=" << radius_ << ", color=" << color_ << ")\n";
    }
    
    void draw() const override {
        std::cout << "[CIRCLE] Drawing " << name_ << " (r=" << radius_ << ", color=" << color_ << ")" << std::endl;
    }
    
    std::string getType() const override {
        return name_;
    }
    
    double getArea() const override {
        return 3.14159 * radius_ * radius_;
    }
    
    void printInfo() const override {
        std::cout << "Circle - Name: " << name_ << ", Radius: " << radius_ 
                  << ", Color: " << color_ << std::endl;
    }
    
    double getRadius() const { return radius_; }
    const std::string& getColor() const { return color_; }
};

class Rectangle : public Shape {
private:
    double width_, height_;
    std::string color_;
    std::string name_;

public:
    Rectangle(double width, double height, const std::string& color = "blue", 
              const std::string& name = "Rectangle") 
        : width_(width), height_(height), color_(color), name_(name) {
        std::cout << "[RECTANGLE] Created: " << name_ << " (" << width_ << "x" << height_ << ", color=" << color_ << ")\n";
    }
    
    void draw() const override {
        std::cout << "[RECTANGLE] Drawing " << name_ << " (" << width_ << "x" << height_ 
                  << ", color=" << color_ << ")" << std::endl;
    }
    
    std::string getType() const override {
        return name_;
    }
    
    double getArea() const override {
        return width_ * height_;
    }
    
    void printInfo() const override {
        std::cout << "Rectangle - Name: " << name_ << ", Width: " << width_ 
                  << ", Height: " << height_ << ", Color: " << color_ << std::endl;
    }
    
    double getWidth() const { return width_; }
    double getHeight() const { return height_; }
};

class Triangle : public Shape {
private:
    double base_, height_;
    std::string color_;
    std::string name_;

public:
    Triangle(double base, double height, const std::string& color = "green", 
             const std::string& name = "Triangle") 
        : base_(base), height_(height), color_(color), name_(name) {
        std::cout << "[TRIANGLE] Created: " << name_ << " (base=" << base_ << ", height=" << height_ << ", color=" << color_ << ")\n";
    }
    
    void draw() const override {
        std::cout << "[TRIANGLE] Drawing " << name_ << " (base=" << base_ << ", height=" << height_ 
                  << ", color=" << color_ << ")" << std::endl;
    }
    
    std::string getType() const override {
        return name_;
    }
    
    double getArea() const override {
        return 0.5 * base_ * height_;
    }
    
    void printInfo() const override {
        std::cout << "Triangle - Name: " << name_ << ", Base: " << base_ 
                  << ", Height: " << height_ << ", Color: " << color_ << std::endl;
    }
    
    double getBase() const { return base_; }
    double getHeight() const { return height_; }
};

// Base proxy class - follows the same interface as Shape
class ShapeProxy : public Shape {
protected:
    std::string proxyName_;

public:
    explicit ShapeProxy(const std::string& name = "GenericProxy") : proxyName_(name) {}
    virtual ~ShapeProxy() = default;
    
    virtual void validateAccess() const {
        std::cout << "[" << proxyName_ << "] Access validation performed\n";
    }
    
    virtual void beforeOperation() const {
        std::cout << "[" << proxyName_ << "] Pre-operation hook\n";
    }
    
    virtual void afterOperation() const {
        std::cout << "[" << proxyName_ << "] Post-operation hook\n";
    }
};

// 1. Remote Proxy - simulates access to remote objects
class RemoteShapeProxy : public ShapeProxy {
private:
    std::unique_ptr<Shape> realShape_;
    std::string serverAddress_;
    double networkDelay_;

public:
    RemoteShapeProxy(std::unique_ptr<Shape> shape, const std::string& server = "localhost", 
                     double delay = 0.1) 
        : ShapeProxy("RemoteProxy"), realShape_(std::move(shape)), 
          serverAddress_(server), networkDelay_(delay) {}
    
    void draw() const override {
        std::cout << "[" << proxyName_ << "] CONNECTING to server " << serverAddress_ << "...\n";
        simulateNetworkDelay();
        std::cout << "[" << proxyName_ << "] REMOTE rendering initiated\n";
        realShape_->draw();
        std::cout << "[" << proxyName_ << "] RENDER complete on " << serverAddress_ << std::endl;
    }
    
    std::string getType() const override {
        std::cout << "[" << proxyName_ << "] FETCHING type from " << serverAddress_ << "...\n";
        simulateNetworkDelay();
        return realShape_->getType();
    }
    
    double getArea() const override {
        std::cout << "[" << proxyName_ << "] REQUESTING area calculation from " << serverAddress_ << "...\n";
        simulateNetworkDelay();
        double area = realShape_->getArea();
        std::cout << "[" << proxyName_ << "] RECEIVED area: " << area << std::endl;
        return area;
    }
    
    void printInfo() const override {
        std::cout << "[" << proxyName_ << "] REQUESTING info from " << serverAddress_ << "...\n";
        simulateNetworkDelay();
        realShape_->printInfo();
    }
    
private:
    void simulateNetworkDelay() const {
        std::this_thread::sleep_for(std::chrono::milliseconds(static_cast<int>(networkDelay_ * 100)));
    }
};

// 2. Virtual Proxy - lazy initialization
class VirtualShapeProxy : public ShapeProxy {
private:
    std::function<std::unique_ptr<Shape>()> shapeFactory_;
    mutable std::unique_ptr<Shape> realShape_;
    mutable bool isShapeCreated_;

public:
    explicit VirtualShapeProxy(std::function<std::unique_ptr<Shape>()> factory) 
        : ShapeProxy("VirtualProxy"), shapeFactory_(std::move(factory)), 
          isShapeCreated_(false) {}
    
    void draw() const override {
        if (!isShapeCreated_) {
            std::cout << "[" << proxyName_ << "] LAZILY creating expensive shape...\n";
            realShape_ = shapeFactory_();
            isShapeCreated_ = true;
        }
        realShape_->draw();
    }
    
    std::string getType() const override {
        if (!isShapeCreated_) {
            realShape_ = shapeFactory_();
            isShapeCreated_ = true;
        }
        return realShape_->getType();
    }
    
    double getArea() const override {
        if (!isShapeCreated_) {
            realShape_ = shapeFactory_();
            isShapeCreated_ = true;
        }
        return realShape_->getArea();
    }
    
    void printInfo() const override {
        if (!isShapeCreated_) {
            realShape_ = shapeFactory_();
            isShapeCreated_ = true;
        }
        realShape_->printInfo();
    }
    
    bool isRealShapeCreated() const { return isShapeCreated_; }
};

// 3. Protection Proxy - controls access based on permissions
class ProtectionProxy : public ShapeProxy {
private:
    std::unique_ptr<Shape> realShape_;
    std::string requiredPermission_;
    std::string currentUserRole_;

public:
    ProtectionProxy(std::unique_ptr<Shape> shape, const std::string& requiredPermission = "public",
                    const std::string& currentUserRole = "guest") 
        : ShapeProxy("ProtectionProxy"), realShape_(std::move(shape)), 
          requiredPermission_(requiredPermission), currentUserRole_(currentUserRole) {}
    
    void draw() const override {
        if (hasAccess()) {
            std::cout << "[" << proxyName_ << "] ACCESS GRANTED - Drawing with protection:\n";
            realShape_->draw();
        } else {
            std::cout << "[" << proxyName_ << "] ACCESS DENIED - Role '" << currentUserRole_ 
                      << "' lacks permission '" << requiredPermission_ << "'" << std::endl;
        }
    }
    
    std::string getType() const override {
        if (hasAccess()) {
            return realShape_->getType();
        } else {
            return "PROTECTED_SHAPE_ACCESS_DENIED";
        }
    }
    
    double getArea() const override {
        if (hasAccess()) {
            std::cout << "[" << proxyName_ << "] CALCULATING area with protection:\n";
            return realShape_->getArea();
        } else {
            std::cout << "[" << proxyName_ << "] AREA ACCESS DENIED\n";
            return -1.0;
        }
    }
    
    void printInfo() const override {
        if (hasAccess()) {
            realShape_->printInfo();
        } else {
            std::cout << "Shape information protected - access denied\n";
        }
    }
    
    void setCurrentUserRole(const std::string& role) {
        currentUserRole_ = role;
    }
    
    bool hasAccess() const {
        if (requiredPermission_ == "public") return true;
        if (requiredPermission_ == "user" && (currentUserRole_ == "user" || currentUserRole_ == "admin")) return true;
        if (requiredPermission_ == "admin" && currentUserRole_ == "admin") return true;
        return false;
    }
};

// 4. Caching Proxy - caches expensive operations
class CachingProxy : public ShapeProxy {
private:
    std::unique_ptr<Shape> realShape_;
    mutable double cachedArea_;
    mutable bool areaCached_;
    mutable std::string cachedType_;
    mutable bool typeCached_;

public:
    explicit CachingProxy(std::unique_ptr<Shape> shape) 
        : ShapeProxy("CachingProxy"), realShape_(std::move(shape)), 
          cachedArea_(0.0), areaCached_(false), cachedType_(""), typeCached_(false) {}
    
    void draw() const override {
        std::cout << "[" << proxyName_ << "] CACHED drawing:\n";
        realShape_->draw();
    }
    
    std::string getType() const override {
        if (!typeCached_) {
            std::cout << "[" << proxyName_ << "] FIRST ACCESS - caching type...\n";
            cachedType_ = realShape_->getType();
            typeCached_ = true;
        } else {
            std::cout << "[" << proxyName_ << "] SECOND+ ACCESS - returning cached type...\n";
        }
        return cachedType_;
    }
    
    double getArea() const override {
        if (!areaCached_) {
            std::cout << "[" << proxyName_ << "] FIRST ACCESS - calculating and caching area...\n";
            cachedArea_ = realShape_->getArea();
            areaCached_ = true;
        } else {
            std::cout << "[" << proxyName_ << "] SECOND+ ACCESS - returning cached area...\n";
        }
        return cachedArea_;
    }
    
    void printInfo() const override {
        realShape_->printInfo();
    }
    
    void clearCache() {
        areaCached_ = false;
        typeCached_ = false;
        cachedArea_ = 0.0;
        cachedType_ = "";
    }
    
    bool isAreaCached() const { return areaCached_; }
    bool isTypeCached() const { return typeCached_; }
};

// 5. Logging Proxy - logs all operations
class LoggingProxy : public ShapeProxy {
private:
    std::unique_ptr<Shape> realShape_;
    std::vector<std::string> operationLog_;

public:
    explicit LoggingProxy(std::unique_ptr<Shape> shape) 
        : ShapeProxy("LoggingProxy"), realShape_(std::move(shape)) {}
    
    void draw() const override {
        addToLog("draw()");
        std::cout << "[" << proxyName_ << "] LOG: Executing draw operation\n";
        realShape_->draw();
    }
    
    std::string getType() const override {
        addToLog("getType()");
        std::cout << "[" << proxyName_ << "] LOG: Getting type\n";
        return realShape_->getType();
    }
    
    double getArea() const override {
        addToLog("getArea()");
        std::cout << "[" << proxyName_ << "] LOG: Getting area\n";
        return realShape_->getArea();
    }
    
    void printInfo() const override {
        addToLog("printInfo()");
        std::cout << "[" << proxyName_ << "] LOG: Printing info\n";
        realShape_->printInfo();
    }
    
    void printLog() const {
        std::cout << "[" << proxyName_ << "] OPERATION LOG (" << operationLog_.size() << " operations):\n";
        for (size_t i = 0; i < operationLog_.size(); ++i) {
            std::cout << "  " << (i + 1) << ". " << operationLog_[i] << std::endl;
        }
    }
    
    size_t getLogSize() const { return operationLog_.size(); }
    
private:
    void addToLog(const std::string& operation) const {
        const_cast<std::vector<std::string>&>(operationLog_).push_back(operation);
    }
};

// 6. Smart Proxy - adds reference counting
class SmartShapeProxy : public ShapeProxy {
private:
    std::unique_ptr<Shape> realShape_;
    mutable int referenceCount_;
    std::string ownerId_;

public:
    SmartShapeProxy(std::unique_ptr<Shape> shape, const std::string& ownerId = "System") 
        : ShapeProxy("SmartProxy"), realShape_(std::move(shape)), 
          referenceCount_(1), ownerId_(ownerId) {}
    
    void draw() const override {
        beforeOperation();
        std::cout << "[" << proxyName_ << "] REF COUNT: " << referenceCount_ 
                  << ", OWNER: " << ownerId_ << std::endl;
        realShape_->draw();
        afterOperation();
    }
    
    std::string getType() const override {
        return realShape_ ? realShape_->getType() : "Null Shape";
    }
    
    double getArea() const override {
        return realShape_ ? realShape_->getArea() : 0.0;
    }
    
    void printInfo() const override {
        std::cout << "SmartShapeProxy - Owner: " << ownerId_ 
                  << ", References: " << referenceCount_ << std::endl;
        if (realShape_) {
            realShape_->printInfo();
        }
    }
    
    int getReferenceCount() const { return referenceCount_; }
    void incrementRefCount() { referenceCount_++; }
    void decrementRefCount() { if (referenceCount_ > 0) referenceCount_--; }
};

// 7. Enhanced Proxy - adds styling and scaling
class EnhancedProxy : public ShapeProxy {
private:
    std::unique_ptr<Shape> realShape_;
    std::string style_;
    double scale_;
    bool visible_;

public:
    EnhancedProxy(std::unique_ptr<Shape> shape, const std::string& style = "default", 
                  double scale = 1.0, bool visible = true) 
        : ShapeProxy("EnhancedProxy"), realShape_(std::move(shape)), 
          style_(style), scale_(scale), visible_(visible) {}
    
    void draw() const override {
        if (!visible_) {
            std::cout << "[" << proxyName_ << "] SHAPE NOT VISIBLE, skipping draw\n";
            return;
        }
        
        std::cout << "[" << proxyName_ << "] ENHANCED drawing with style: " << style_ 
                  << " and scale: " << scale_ << std::endl;
        realShape_->draw();
    }
    
    std::string getType() const override {
        return style_ + " " + realShape_->getType();
    }
    
    double getArea() const override {
        return realShape_->getArea() * scale_ * scale_;
    }
    
    void printInfo() const override {
        std::cout << "EnhancedProxy - Style: " << style_ 
                  << ", Scale: " << scale_ 
                  << ", Visible: " << (visible_ ? "Yes" : "No") << std::endl;
        realShape_->printInfo();
    }
    
    // Setters
    void setStyle(const std::string& style) { style_ = style; }
    void setScale(double scale) { scale_ = scale; }
    void setVisible(bool visible) { visible_ = visible; }
    
    // Getters
    std::string getStyle() const { return style_; }
    double getScale() const { return scale_; }
    bool isVisible() const { return visible_; }
};

// 8. Proxy Factory - creates different types of proxies
class ShapeProxyFactory {
public:
    // Factory methods for different proxy types
    static std::unique_ptr<Shape> createRemoteProxy(std::unique_ptr<Shape> shape, 
                                                   const std::string& server = "localhost", 
                                                   double delay = 0.1) {
        return std::make_unique<RemoteShapeProxy>(std::move(shape), server, delay);
    }
    
    static std::unique_ptr<Shape> createVirtualProxy(std::function<std::unique_ptr<Shape>()> factory) {
        return std::make_unique<VirtualShapeProxy>(std::move(factory));
    }
    
    static std::unique_ptr<Shape> createProtectionProxy(std::unique_ptr<Shape> shape, 
                                                       const std::string& permission = "public",
                                                       const std::string& role = "guest") {
        return std::make_unique<ProtectionProxy>(std::move(shape), permission, role);
    }
    
    static std::unique_ptr<Shape> createCachingProxy(std::unique_ptr<Shape> shape) {
        return std::make_unique<CachingProxy>(std::move(shape));
    }
    
    static std::unique_ptr<Shape> createLoggingProxy(std::unique_ptr<Shape> shape) {
        return std::make_unique<LoggingProxy>(std::move(shape));
    }
    
    static std::unique_ptr<Shape> createSmartProxy(std::unique_ptr<Shape> shape, 
                                                  const std::string& owner = "System") {
        return std::make_unique<SmartShapeProxy>(std::move(shape), owner);
    }
    
    static std::unique_ptr<Shape> createEnhancedProxy(std::unique_ptr<Shape> shape,
                                                     const std::string& style = "default",
                                                     double scale = 1.0, 
                                                     bool visible = true) {
        return std::make_unique<EnhancedProxy>(std::move(shape), style, scale, visible);
    }
    
    // Create a complete shape system with multiple proxies
    static std::unique_ptr<Shape> createFullFeaturedShape(std::unique_ptr<Shape> baseShape,
                                                         const std::string& server = "",
                                                         const std::string& permission = "user",
                                                         const std::string& role = "user",
                                                         const std::string& style = "default",
                                                         double scale = 1.0) {
        // Start with base shape, then apply multiple proxies
        auto shape = std::make_unique<EnhancedProxy>(std::move(baseShape), style, scale, true);
        shape = std::make_unique<ProtectionProxy>(std::move(shape), permission, role);
        if (!server.empty()) {
            shape = std::make_unique<RemoteShapeProxy>(std::move(shape), server, 0.05);
        }
        return std::make_unique<LoggingProxy>(std::move(shape));
    }
};

// 9. Complex Proxy: Multi-function Proxy that combines multiple features
class MultiFeatureProxy : public ShapeProxy {
private:
    std::unique_ptr<Shape> realShape_;
    bool enableLogging_, enableCaching_, enableProtection_;
    std::string permission_, userRole_;
    std::vector<std::string> logEntries_;
    mutable double cachedArea_;
    mutable bool areaCached_;
    mutable bool cachedTypeCached_;
    mutable std::string cachedTypeValue_;

public:
    MultiFeatureProxy(std::unique_ptr<Shape> shape, 
                     bool logging = false, bool caching = false, bool protection = false,
                     const std::string& permission = "user", const std::string& userRole = "guest") 
        : ShapeProxy("MultiFeatureProxy"), realShape_(std::move(shape)),
          enableLogging_(logging), enableCaching_(caching), enableProtection_(protection),
          permission_(permission), userRole_(userRole),
          cachedArea_(0.0), areaCached_(false), cachedTypeCached_(false), cachedTypeValue_("") {}
    
    void draw() const override {
        logOperation("draw()");
        if (enableProtection_ && !hasAccess()) {
            std::cout << "[" << proxyName_ << "] PROTECTED ACCESS DENIED - Cannot draw\n";
            return;
        }
        std::cout << "[" << proxyName_ << "] MULTI-FEATURE drawing:\n";
        realShape_->draw();
    }
    
    std::string getType() const override {
        logOperation("getType()");
        if (enableProtection_ && !hasAccess()) {
            return "PROTECTED_SHAPE_ACCESS_DENIED";
        }
        
        if (enableCaching_ && !cachedTypeCached_) {
            std::cout << "[" << proxyName_ << "] CACHING type info...\n";
            cachedTypeValue_ = realShape_->getType();
            cachedTypeCached_ = true;
            return cachedTypeValue_;
        } else if (enableCaching_ && cachedTypeCached_) {
            std::cout << "[" << proxyName_ << "] USING cached type info...\n";
            return cachedTypeValue_;
        }
        
        return realShape_->getType();
    }
    
    double getArea() const override {
        logOperation("getArea()");
        if (enableProtection_ && !hasAccess()) {
            return -1.0;
        }
        
        if (enableCaching_ && !areaCached_) {
            std::cout << "[" << proxyName_ << "] CACHING area calculation...\n";
            cachedArea_ = realShape_->getArea();
            areaCached_ = true;
            return cachedArea_;
        } else if (enableCaching_ && areaCached_) {
            std::cout << "[" << proxyName_ << "] USING cached area...\n";
            return cachedArea_;
        }
        
        return realShape_->getArea();
    }
    
    void printInfo() const override {
        logOperation("printInfo()");
        if (enableProtection_ && !hasAccess()) {
            std::cout << "Shape information protected - access denied\n";
            return;
        }
        realShape_->printInfo();
    }
    
    // Multi-feature management methods
    void enableLogging() { enableLogging_ = true; }
    void enableCaching() { enableCaching_ = true; }
    void enableProtection() { enableProtection_ = true; }
    
    void setUserRole(const std::string& role) { userRole_ = role; }
    
    size_t getLogSize() const { return logEntries_.size(); }
    void clearCache() { areaCached_ = false; cachedTypeCached_ = false; }
    
private:
    void logOperation(const std::string& operation) {
        if (enableLogging_) {
            logEntries_.push_back(operation);
            std::cout << "[" << proxyName_ << "] LOG: " << operation << std::endl;
        }
    }
    
    bool hasAccess() const {
        if (permission_ == "public") return true;
        if (permission_ == "user" && (userRole_ == "user" || userRole_ == "admin")) return true;
        if (permission_ == "admin" && userRole_ == "admin") return true;
        return false;
    }
};

// 10. Proxy Manager - manages collections of proxied shapes
class ProxyManager {
private:
    std::vector<std::unique_ptr<Shape>> shapes_;
    std::string managerName_;

public:
    explicit ProxyManager(const std::string& name = "ShapeProxyManager") : managerName_(name) {}
    
    void addShape(std::unique_ptr<Shape> shape) {
        shapes_.push_back(std::move(shape));
    }
    
    void renderAll() const {
        std::cout << "[" << managerName_ << "] Rendering all " << shapes_.size() << " shapes:\n";
        for (size_t i = 0; i < shapes_.size(); ++i) {
            std::cout << "Shape " << (i + 1) << ": ";
            shapes_[i]->draw();
        }
    }
    
    double calculateTotalArea() const {
        double total = 0.0;
        for (const auto& shape : shapes_) {
            double area = shape->getArea();
            if (area >= 0) { // Ignore negative values (protection denied)
                total += area;
            }
        }
        return total;
    }
    
    size_t getShapeCount() const { return shapes_.size(); }
    void clear() { shapes_.clear(); }
    
    void printAllInfo() const {
        std::cout << "[" << managerName_ << "] Information for all shapes:\n";
        for (size_t i = 0; i < shapes_.size(); ++i) {
            std::cout << "Shape " << (i + 1) << ": \n";
            shapes_[i]->printInfo();
            std::cout << std::endl;
        }
    }
    
    Shape* getShape(size_t index) const {
        if (index < shapes_.size()) {
            return shapes_[index].get();
        }
        return nullptr;
    }
    
    std::string getManagerName() const { return managerName_; }
};

#endif // PROXY_PATTERNS_H